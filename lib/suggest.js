...
function suggest(value) {
  ...
  var allPositions = groups.map(function (group) {
    return group
      .split('')
      .filter(function (char) {
        return valueLowerCase.includes(char)
      })
      .map(function (char) {
        return group.indexOf(char)
      })
  })
  var suggestions = []
  var weighted = {}
  ...
  while (++index < length) {
    ...
    while (++offset < count) {
      group = groups[offset]
      pseudoPosition = allPositions[offset].find(function (pos) {
        return pos === index
      })

      // Allow if any char in group happens to match index in value
      if (pseudoPosition === undefined) {
        continue
      }
      ...
      while (++otherOffset < otherCount) {
        if (otherOffset !== pseudoPosition) {
          otherCharacter = group.charAt(otherOffset)
          if (otherCharacter === insensitive) {
            continue
          }
