...
function suggest(value) {
  ...
  console.log({edits_no_keyboard: JSON.stringify(edits)})
  ...
  console.log({edits_no_doubles: JSON.stringify(edits)})
  ...
  console.log({edits_final: JSON.stringify(edits), values, currentCase})
  ...
  while (!suggestions.length && previous < max) {
    next = previous + size
    //console.log({firstLevel: firstLevel.slice(previous, next), previous, next})
  ...
  log_sort(suggestions[0], suggestions[1])
...
function generate(context, memory, words, edits) {
  ...
  while (++index < length) {
    ...
    while (++position < count) {
      ...
      if (edits) {
        // console.log({character, word, position, upper, new_count: result.slice(logCacheEnd).length, new_result: JSON.stringify(result.slice(logCacheEnd))})
        logCacheEnd = result.length
      }
    }
  }
